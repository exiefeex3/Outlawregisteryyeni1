...
if(!message.member.roles.get("707712303889645648") && !message.member.hasPermission('ADMINISTRATOR'))  return message.channel.send(new Discord.RichEmbed() .setDescription(`Bu komutu Kullanmaya Hakkın yok :x:`).setAuthor(message.author.tag ,message.author.avatarURL).setColor("RANDOM")).then(m => m.delete(5000));
    let kullanici = message.mentions.members.first()
    let chan = client.channels.get("712052853929279550")
    var muteTime = args[1];
  ...
  message.react('707712303889645648')
...
.setColor("RANDOM"))    
        ...
        setTimeout(() => {
    let chan1 = client.channels.get("712052853929279550")
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ["vmute","sesmute","seslisustur", "mute"],
    permLevel: 0
};
