...
let rochelle1 = message.mentions.users.first() || client.users.get(args.join(' ')) || message.guild.members.find(c=> c.id === args[0])
  if (!rochelle1) return message.channel.sendEmbed(new Discord.RichEmbed().addField(`${emoji}  Bilgi` , `   Bir kullanıcı etiketlemeli ve ya id girmelisin!`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp());
...
rochelle.setNickname(`${isim} | ${yas}`)
  rochelle.addRole('676538917956091934'); // kadın 1
  rochelle.addRole('725898929136205924'); // kadın 1
  rochelle.addRole('727908151965515837'); // kadın 1
  rochelle.addRole('727908151965515837'); // kadın 1
  rochelle.addRole('727951641814499388')
  rochelle.addRole('727908151046701116')

  rochelle.removeRole('698417266684854292'); // erkek 1
  rochelle.removeRole('725898930369331532')//TlhaMert Youtube Kanalı : https://youtube.com/c/TlhaMert
...
db.add(`kadın.${message.author.id}`, 1);
  ...
  .setDescription(`** ${rochelle.user} Adlı Üye <@&727908151965515837> <@&727951641814499388> <@&727908151046701116> Rolüyle Kayıt Oldu.** `)
